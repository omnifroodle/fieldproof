...
| Capella AI Data Plane (paid; not used by the demo) | Older material, including earlier versions of these docs, calls it Capella AI Services. Six parts: **Model Service** (deploy LLMs and embedding models "in Capella"; Enterprise Support only; the embedding models are described as producing "vector representations of text data" — no image model is listed), **Agent Catalog**, **Agent Memory** (Enterprise Support only), **AI Functions**, **MCP Server**, **Data Processing Service** (Enterprise Support only). Checked 2026-09-24. Corrects an earlier row here that said "auto-embeddings on write": see Data Processing below. | https://docs.couchbase.com/ai/get-started/intro.html , https://docs.couchbase.com/ai/build/model-service/model-service.html |
| AI Data Plane: AI Functions | Ten SQL++ functions: `ai_sentiment`, `ai_summary`, `ai_classification`, `ai_extraction`, `ai_corrected_grammar`, `ai_generated_text`, `ai_masked` (hides PII such as names and emails), `ai_similarity`, `ai_translation`, `ai_completion`. Backed by a Model Service model, OpenAI, or Amazon Bedrock. **Requires a paid Capella cluster on Couchbase Server 8.0+, Developer Pro or Enterprise support, the Query Service, and multiple availability zones** — not the free tier. Checked 2026-09-24. | https://docs.couchbase.com/ai/build/ai-functions.html |
| AI Data Plane: Data Processing Service | Runs **Workflows you start**; it does not vectorize documents automatically as they are written. Three workflow types: unstructured data from S3 (PDF, JPEG, PNG), pre-processed JSON from S3, and data already in a Capella cluster. Embedding model from the Model Service, OpenAI, or Amazon Bedrock. Limits: 100 MB per file, 10,000 files per Workflow. The page does not say where vectors are written or whether an index is created. Checked 2026-09-24. | https://docs.couchbase.com/ai/build/vectorization-service/data-processing.html |
| Couchbase MCP Server | Self-hosted (PyPI or Docker) MCP server for Capella or self-managed clusters. Tools include `run_sql_plus_plus_query`, schema listing, and document get/insert/upsert/replace/delete. Offers a read-only mode and per-tool disabling; read-only is not the default. Enterprise support comes with an AI Data Plane licence. Checked 2026-09-24. | https://docs.couchbase.com/mcp-server/get-started/overview.html |
...
| Foundation Models (Phase 7) | **iOS 26.0+.** `SystemLanguageModel.default.availability` → `.available` or `.unavailable(UnavailableReason)` with cases `.deviceNotEligible`, `.appleIntelligenceNotEnabled`, `.modelNotReady`. `LanguageModelSession(model:tools:instructions:)` (String `instructions` overload is `@_disfavoredOverload`; there is also an `Instructions?` overload). `try await session.respond(to: String, options: GenerationOptions()) async throws -> LanguageModelSession.Response<String>`, text in `.content`. `GenerationOptions` has `temperature: Double?` and `maximumResponseTokens: Int?`. Needs a device with Apple Intelligence (iPhone 15 Pro or newer). | https://developer.apple.com/documentation/foundationmodels , verified 2026-09-22 against `iPhoneOS27.0.sdk/System/Library/Frameworks/FoundationModels.framework/Modules/FoundationModels.swiftmodule/arm64e-apple-ios.swiftinterface` |
| Speech, `SFSpeechRecognizer` (Phase 7) | **iOS 10+, not deprecated in the iOS 27 SDK.** `SFSpeechRecognizer.requestAuthorization { SFSpeechRecognizerAuthorizationStatus }`; `init(locale:)`; `supportsOnDeviceRecognition` (iOS 13+); `recognitionTask(with:resultHandler:)` → `SFSpeechRecognitionTask`. Request: `SFSpeechAudioBufferRecognitionRequest` with `shouldReportPartialResults`, **`requiresOnDeviceRecognition`** (iOS 13+, the property that keeps audio off the network), `append(_ buffer: AVAudioPCMBuffer)` / `appendAudioPCMBuffer(_:)`, `endAudio()`. Result: `SFSpeechRecognitionResult.bestTranscription.formattedString`, `isFinal`. Info.plist: `NSSpeechRecognitionUsageDescription` **and** `NSMicrophoneUsageDescription`. | https://developer.apple.com/documentation/speech/sfspeechrecognizer , https://developer.apple.com/documentation/speech/sfspeechaudiobufferrecognitionrequest , verified 2026-09-22 against `iPhoneOS27.0.sdk/System/Library/Frameworks/Speech.framework/Headers/` |
| Speech, `SpeechAnalyzer` / `SpeechTranscriber` (not used) | The iOS 26 replacement API is present in the SDK (`Speech.swiftmodule`, `SpeechTranscriber(locale:preset:)` with presets `.transcription`, `.progressiveTranscription`, …). FieldProof targets iOS 17, so it uses `SFSpeechRecognizer`, which needs no availability fork and no model asset download. | https://developer.apple.com/documentation/speech/speechanalyzer , SDK interface, 2026-09-22 |
| `AVAudioEngine` microphone tap | `engine.inputNode.outputFormat(forBus: 0)`, `installTap(onBus:bufferSize:format:block:)`, `removeTap(onBus:)`, `engine.prepare()`, `engine.start()`, `engine.stop()`. Session: `AVAudioSession.sharedInstance().setCategory(.record, mode: .measurement, options: .duckOthers)` then `setActive(_:options:)`. | https://developer.apple.com/documentation/avfaudio/avaudioengine , https://developer.apple.com/documentation/avfaudio/avaudiosession |
...
### 7.5 Stretch findings (Phase 7, 2026-09-22)

| Finding | Evidence | Consequence |
|---|---|---|
| **Foundation Models works on the device and not on the simulator.** On the simulator `SystemLanguageModel.default.availability` reports `.available`, and then `respond(to:)` throws `GenerationError` wrapping `com.apple.SensitiveContentAnalysisML` code 15 → `ModelManagerServices.ModelManagerError` code 1001: the safety assets are not installed. On the paired iPhone 15 Pro Max the same code passed in **4.455 s** (`ReportSummaryTests`, 11/11 on device). The same prompt on macOS 27 returned "Tree across far lane on bridge approach." | `xcodebuild test` on both, plus a standalone `swift` script on the Mac | `ReportSummary.unavailableReason` returns a simulator-specific line instead of attempting generation. Budget ~4.5 s: the app writes the summary in the background while the crew member reviews. |
| **The on-device speech recognizer cannot initialize on the simulator.** With permissions granted and `supportsOnDeviceRecognition == true`, `recognitionTask` fails immediately: "Failed to initialize recognizer". | Simulator, iOS 26.4 | `VoiceNotes` shows the error under the button. Dictation has to be demonstrated on a device. |
| The system's own speech-permission alert says "Speech data from this app will be sent to Apple to process your requests" even when the request sets `requiresOnDeviceRecognition`. | Simulator | Expect the question on stage; the answer is that the property keeps the audio on the phone. The alert text is Apple's, not ours. |
| **`_changes` with `feed=longpoll` rejects `since=now`** ("Invalid sequence"). Starting at `since=0` returns the history at once and hands back a usable `last_seq` to continue from. | Live probe | `lib/appservices.js` defaults to `since = '0'`. |
| The changes feed emits **one event per document**, so seeding 30 reports produced 30 events in ~15 s. | Live | The browser coalesces events with a 400 ms timer; one redraw, not thirty. |
| End to end: filing a report on the phone reached the open dashboard **in about 3 seconds** with no polling (the 15 s interval is only a fallback). | Live, timestamped stream log | Replaces the old 3 s poll. |
